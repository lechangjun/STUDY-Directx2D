...
void InitWindow(HINSTANCE hInstance, int ShowWnd)
{
	//Register Class
	{
		WNDCLASSEX wndClass; //확장 클래스
		wndClass.cbSize = sizeof(WNDCLASSEX); //크기
		wndClass.style = CS_HREDRAW | CS_VREDRAW; //수평/수직으로 다시 그리기
		wndClass.lpfnWndProc = WndProc;
		wndClass.cbClsExtra = NULL; //클래스에 무언가가 추가됐을 때
		...
		WORD check = RegisterClassEx(&wndClass); //클래스 등록
		assert(check != 0);
	}
	...
	//Create Handle
	{
		...
		(
			...
			Title.c_str(),
			WS_OVERLAPPEDWINDOW,
			CW_USEDEFAULT, CW_USEDEFAULT, //X Y 위치
			Width, Height, //넓이 높이
			NULL,
			NULL,
			hInstance,
